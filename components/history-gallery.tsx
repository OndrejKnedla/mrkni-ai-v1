...
export function HistoryGallery({ onSelectItem }: HistoryGalleryProps) {
  const { history, loading, clearHistory, removeFromHistory, refreshHistory } = useHistory()

  // Create a state to store unique items
  ...
  useEffect(() => {
    ...
    // Process each item, only adding it if its ID hasn't been seen before
    ...
    console.log(`Processed ${history.length} items into ${uniqueItemsList.length} unique items`)
    ...
    return (
      <GlassCard className="w-full">
        <GlassCardHeader>
          ...
          <GlassCardDescription>Loading your generation history...</GlassCardDescription>
        </GlassCardHeader>
        <GlassCardContent>
  ...
  if (uniqueItems.length === 0) {
    return (
      <GlassCard className="w-full">
        <GlassCardHeader>
          <GlassCardTitle>Generation History</GlassCardTitle>
          <GlassCardDescription>Your generation history will appear here</GlassCardDescription>
        </GlassCardHeader>
        <GlassCardContent>
          <div className="flex flex-col items-center justify-center h-40 text-center">
  ...
  return (
    <GlassCard className="w-full">
      <GlassCardHeader className="flex flex-row items-center justify-between">
        <div>
          <GlassCardTitle>Generation History</GlassCardTitle>
          <GlassCardDescription>Your previous generations</GlassCardDescription>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-3 gap-4">
            {uniqueItems.map((item) => (
              <div key={`unique-${item.id}`} className="relative group">
                <div
                  className="relative aspect-square rounded-md overflow-hidden cursor-pointer bg-black/30"
                  onClick={() => onSelectItem(item)}
                >
                  {/* Video content */}
                  {item.type === 'video' && item.output && item.output.length > 0 ? (
                    <>
                      <video
                        src={item.output[0]}
